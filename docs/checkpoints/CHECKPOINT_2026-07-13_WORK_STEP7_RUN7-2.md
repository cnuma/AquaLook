...
- Branche de travail cible : `work/step7-run7-2`
...
- `src/test_equipment_orchestrator.cpp` : nouveau banc déterministe ;
- `platformio.ini` : exclusion du banc des environnements nominaux et ajout de l’environnement dédié ;
- ce checkpoint.
...
```powershell
pio run -e test_equipment_orchestrator -t upload --upload-port COM3
